  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnalyticsData> => {
      ...
      const [txRes, transfersRes, catRes] = await Promise.all([
        ...
        supabase.from('transfers').select('amount').gte('date', startDate.toISOString().split('T')[0]),
      ...
      if (groupBy === 'day') {
        ...
        return { income, expense, savings: income - expense, byCategory, byAccount, byDay, byMonth, transfers: { count: transfersRes.data?.length ?? 0, total: (transfersRes.data ?? []).reduce((s, t) => s + Number(t.amount), 0) }, transactions, categories };
      } else {
